...
Const
 ...
 //set ConstantID values for Specialisation Constants
  CI_USE_OBJECTID  : Integer = 0;
  ...
  TvgAPI_Version = (VG_API_VERSION,
    ...
    TVgCommandPoolCreateFlag=  Set of
     ...
     TvgPrimitiveTopology=
      ...
      //see TvgDynamicStateBit

     TvgDynamicStateTypeCORE=  Set of
      ( //CORE
       DT_VIEWPORT,
       DT_SCISSOR,
       DT_LINE_WIDTH,
       ...
       DT_BLEND_CONSTANTS,
       DT_DEPTH_BOUNDS,
       DT_STENCIL_COMPARE_MASK,
       DT_STENCIL_WRITE_MASK,
       DT_STENCIL_REFERENCE
      );

     TvgDynamicStateType13=  Set of
       //1.3 and specific extensions
       (
       DT_CULL_MODE,
       DT_FRONT_FACE,
       DT_PRIMITIVE_TOPOLOGY,
       DT_VIEWPORT_WITH_COUNT,
       DT_SCISSOR_WITH_COUNT,
       DT_VERTEX_INPUT_BINDING_STRIDE,
       DT_DEPTH_TEST_ENABLE,
       DT_DEPTH_WRITE_ENABLE,
       DT_DEPTH_COMPARE_OP,
       DT_DEPTH_BOUNDS_TEST_ENABLE,
       DT_STENCIL_TEST_ENABLE,
       DT_STENCIL_OP,
       DT_RASTERIZER_DISCARD_ENABLE,
       DT_DEPTH_BIAS_ENABLE,
       DT_PRIMITIVE_RESTART_ENABLE,
       DT_VIEWPORT_W_SCALING,
       DT_DISCARD_RECTANGLE,
       DT_SAMPLE_LOCATIONS,
       DT_VIEWPORT_SHADING_RATE_PALETTE,
       DT_VIEWPORT_COARSE_SAMPLE_ORDER,
       DT_EXCLUSIVE_SCISSOR,
       DT_FRAGMENT_SHADING_RATE,
       DT_LINE_STIPPLE,
       DT_RAY_TRACING_PIPELINE_STACK_SIZE,
       DT_VERTEX_INPUT,
       DT_PATCH_CONTROL_POINTS,
       DT_LOGIC_OP,
       DT_COLOR_WRITE_ENABLE
      );
    ...
    //cant be a set as it is too large >4k
     ...
     TvgAttachmentType=
      ( atNone,
        ...
        atColour,
        atDepthStencil,
  //      atHighPrecisionDepth,
        atMSAA,          //Multi sampling AntiAliasing attachment
      //  atImageStorage,        //attachment to handle Object selection from the an image
        atCustom         //a manual setup attachment resource
      );

  TvgImageFormatProps = record
     Format     : TvkFormat;
     ...
     TvgImageUsageFlagBits= Set Of
      (
       ...
       IU_RESERVED_16_BIT_HUAWEI,
       IU_RESERVED_27_BIT_HUAWEI,
       IU_INVOCATION_MASK_BIT_HUAWEI,

       IU_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT,
       IU_SAMPLE_WEIGHT_BIT_QCOM,
       IU_SAMPLE_BLOCK_MATCH_BIT_QCOM,
       IU_HOST_TRANSFER_BIT,
       IU_TENSOR_ALIASING_BIT_ARM,
       IU_RESERVED_24_BIT_COREAVI,
       IU_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR,
       IU_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR,
       IU_TILE_MEMORY_BIT_QCOM,
       IU_RESERVED_28_BIT_EXT,
       IU_RESERVED_29_BIT_KHR,
       IU_RESERVED_30_BIT_KHR
      );
     ...
     TvgBufferUsageFlagBits=  Set Of
      (
       ...
       BU_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT,
       BU_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT,
       BU_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT,
       BU_MICROMAP_STORAGE_BIT_EXT,
       BU_EXECUTION_GRAPH_SCRATCH_BIT_AMDX,
       BU_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT,
       BU_TILE_MEMORY_BIT_QCOM,
       BU_DESCRIPTOR_HEAP_BIT_EXT
      );
...
//       SS_ALL_GRAPHICS, doesn't work in set
 ...
 //      SS_ALL,               doesn't work in set
     ...
     TvgDescriptorSetLayoutCreateFlagBits= Set Of
      (
       ...
       DSL_BUFFER_BIT_EXT,
       DSL_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT,
       DSL_PER_STAGE_BIT_NV,
       DSL_INDIRECT_BINDABLE_BIT_NV
 ...
 TvgTextureFileType = (FT_UNKNOWN,
     ...
     TvgSamplerReductionMode=
      ...
      TvgCommandBufferState = ( cbsInitial,
                                cbsRecording,
                                cbsExecutable,
                                cbsPending,
                                cbsInvalid
      ...
      (
        RP_CUSTOM,
        RP_SCREEN,               //used to render data to the screen
        RP_BACKBUFFER,          //render to Back Buffer Need aonther render pass
        RP_BACKBUFFERTOSCREEN   //used to copy OffScreen image to the Screen
      );

      TvgSubPassSet = Set of
      ...
      TvgSubPassMode =
      ( SPT_SIMPLE,              //used to render data straight to the screen
        SPT_G_BUFFER,
        SPT_LIGHTING,
        SPT_TRANSPARENCY,
        SPT_POSTPROCESS,
        SPT_SHADOW,
        SPT_OFFSCREENTOSCREEN,   //used to copy OffScreen image to the Screen
        SPT_UI,                   //used to add User info Cursor/text to screen
        SPT_CUSTOM
      );

      TvgSpecialisationType =
      ( TS_BOOLEAN,
        TS_INTEGER,
        TS_SINGLE     //may use others later
      );

      TvgDescriptorDataFlow = Set of
      ( DF_UP,
        DF_DOWN,
        DF_SAMPLING
        );
 ...
 TvgSceneState =
   ...
   TvgDescriptorBindingMode = ( vgdbmSingle,          //only a single descriptor fBindingCount = 1
                                vgdbmFixedArray,
                                vgdbmVariableArray,
                                vgdbmRuntimeArray
                              );

  TvgDescriptorIndexingOption = ( dioRuntimeArray,
                                  dioVariableDescriptorCount,    // VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT
                                  dioPartiallyBound,             // VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT
                                  dioUpdateAfterBind,            // VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT
                                  dioUpdateUnusedWhilePending    // VK_DESCRIPTOR_BINDING_UPDATE_UNUSED_WHILE_PENDING_BIT
                                );
  TvgDescriptorIndexingOptions = set of TvgDescriptorIndexingOption;

  TvgDescriptorWriteMode = (  vgdmWriteWholeBinding,
  ...
  TvgVulkanVersion = record
    Major: UInt32;
    Minor: UInt32;
    Patch: UInt32;
    class function FromPacked(const V: UInt32): TvgVulkanVersion; static;
    class function Make(const AMajor, AMinor, APatch: UInt32): UInt32; static;
    function ToPacked: UInt32;
    function ToString: string; // "major.minor.patch"
    function IsAtLeast(const Other: TvgVulkanVersion): Boolean;
  end;

// Simple key structure for hashing TVkRenderPassCreateInfo
  TRenderPassKey = record
    AttachmentCount: Cardinal;
    AttachmentFormats: array of TVkFormat;  // Simplified; expand for full desc
    SubpassCount: Cardinal;
    // Add more fields as needed: sample counts, load ops, etc.
    function ComputeHash: Cardinal;
  end;
      ...
      Function GetVGFormatAsString(Value:TvgFormat):String;
  ...
  function GetPixelSizeInBytes(const aFormat: TVkFormat): Integer;

  { Safe pointer validation function for TObject instances }

implementation


    { Vk version encoding rules used by Vulkan:
      - Major: bits 31..22  (10 bits)
      - Minor: bits 21..12  (10 bits)
      - Patch: bits 11..0   (12 bits)
      This matches VK_MAKE_VERSION / VK_VERSION_MAJOR / VK_VERSION_MINOR / VK_VERSION_PATCH macros.
    }

    class function TvgVulkanVersion.Make(const AMajor, AMinor, APatch: UInt32): UInt32;
    begin
      // Equivalent to VK_MAKE_VERSION(major, minor, patch)
      Result := (AMajor shl 22) or (AMinor shl 12) or (APatch);
    end;

    function TvgVulkanVersion.ToPacked: UInt32;
    begin
      Result := Make(Major, Minor, Patch);
    end;

    class function TvgVulkanVersion.FromPacked(const V: UInt32): TvgVulkanVersion;
    begin
      Result.Major := V shr 22;
      Result.Minor := (V shr 12) and $3FF; // 10 bits
      Result.Patch := V and $FFF; // 12 bits
    end;

    function TvgVulkanVersion.ToString: string;
    begin
      Result := SysUtils.Format('%d.%d.%d', [Major, Minor, Patch]);
    end;

    function TvgVulkanVersion.IsAtLeast(const Other: TvgVulkanVersion): Boolean;
    begin
      // Compare by packing and doing numeric compare is safe because of bit layout,
      // but explicit compare is clearer:
...
function GetPixelSizeInBytes(const aFormat: TVkFormat): Integer;
var
  blockWidth, blockHeight, bytesPerBlock: Integer;
begin
  Result := 0;
  ...
  case aFormat of
    ...
    // ETC2 RGB8 / RGB8A1: 8 bytes per 4x4 block
    ...
    VK_FORMAT_ASTC_5x4_UNORM_BLOCK, VK_FORMAT_ASTC_5x4_SRGB_BLOCK:
      begin blockWidth := 5; blockHeight := 4; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_5x5_UNORM_BLOCK, VK_FORMAT_ASTC_5x5_SRGB_BLOCK:
      begin blockWidth := 5; blockHeight := 5; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_6x5_UNORM_BLOCK, VK_FORMAT_ASTC_6x5_SRGB_BLOCK:
      begin blockWidth := 6; blockHeight := 5; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_6x6_UNORM_BLOCK, VK_FORMAT_ASTC_6x6_SRGB_BLOCK:
      begin blockWidth := 6; blockHeight := 6; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_8x5_UNORM_BLOCK, VK_FORMAT_ASTC_8x5_SRGB_BLOCK:
      begin blockWidth := 8; blockHeight := 5; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_8x6_UNORM_BLOCK, VK_FORMAT_ASTC_8x6_SRGB_BLOCK:
      begin blockWidth := 8; blockHeight := 6; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_8x8_UNORM_BLOCK, VK_FORMAT_ASTC_8x8_SRGB_BLOCK:
      begin blockWidth := 8; blockHeight := 8; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_10x5_UNORM_BLOCK, VK_FORMAT_ASTC_10x5_SRGB_BLOCK:
      begin blockWidth := 10; blockHeight := 5; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_10x6_UNORM_BLOCK, VK_FORMAT_ASTC_10x6_SRGB_BLOCK:
      begin blockWidth := 10; blockHeight := 6; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_10x8_UNORM_BLOCK, VK_FORMAT_ASTC_10x8_SRGB_BLOCK:
      begin blockWidth := 10; blockHeight := 8; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_10x10_UNORM_BLOCK, VK_FORMAT_ASTC_10x10_SRGB_BLOCK:
      begin blockWidth := 10; blockHeight := 10; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_12x10_UNORM_BLOCK, VK_FORMAT_ASTC_12x10_SRGB_BLOCK:
      begin blockWidth := 12; blockHeight := 10; bytesPerBlock := 16; end;
    VK_FORMAT_ASTC_12x12_UNORM_BLOCK, VK_FORMAT_ASTC_12x12_SRGB_BLOCK:
      begin blockWidth := 12; blockHeight := 12; bytesPerBlock := 16; end;
  else
    Result := 0; // Unknown format
  end;

  // If it's a compressed format, calculate effective bytes-per-pixel
  if bytesPerBlock > 0 then
    Result := bytesPerBlock div (blockWidth * blockHeight);
end;
  ...
  *)
   ...
   //     If TestValue(VK_PIPELINE_CREATE_RESERVED_27_BIT_EXT)                                  then include(Result, PC_RESERVED_27_BIT_EXT) ;
      ...
      Begin
        ...
        If (DSL_BUFFER_BIT_EXT in Value)                     then Result := Result OR TVkDescriptorSetLayoutCreateFlags(VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT) ;
        If (DSL_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT in Value)then Result := Result OR TVkDescriptorSetLayoutCreateFlags(VK_DESCRIPTOR_SET_LAYOUT_CREATE_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT) ;
        If (DSL_PER_STAGE_BIT_NV in Value)                   then Result := Result OR TVkDescriptorSetLayoutCreateFlags(VK_DESCRIPTOR_SET_LAYOUT_CREATE_PER_STAGE_BIT_NV) ;
        If (DSL_INDIRECT_BINDABLE_BIT_NV in Value)           then Result := Result OR TVkDescriptorSetLayoutCreateFlags(VK_DESCRIPTOR_SET_LAYOUT_CREATE_INDIRECT_BINDABLE_BIT_NV) ;
      ...
      Begin
        ...
        If TestValue(VK_DESCRIPTOR_SET_LAYOUT_CREATE_DESCRIPTOR_BUFFER_BIT_EXT)       then include(Result, DSL_BUFFER_BIT_EXT) ;
        If TestValue(VK_DESCRIPTOR_SET_LAYOUT_CREATE_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT)       then include(Result, DSL_EMBEDDED_IMMUTABLE_SAMPLERS_BIT_EXT) ;
        If TestValue(VK_DESCRIPTOR_SET_LAYOUT_CREATE_PER_STAGE_BIT_NV)       then include(Result, DSL_PER_STAGE_BIT_NV) ;
        If TestValue(VK_DESCRIPTOR_SET_LAYOUT_CREATE_INDIRECT_BINDABLE_BIT_NV)       then include(Result, DSL_INDIRECT_BINDABLE_BIT_NV) ;
      End;
      ...
      Begin
        ...
        If (BU_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT in Value)                     then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT) ;


        If (BU_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT in Value)      then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT) ;
        If (BU_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT  in Value) then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT) ;
        If (BU_MICROMAP_STORAGE_BIT_EXT  in Value)               then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_MICROMAP_STORAGE_BIT_EXT) ;
        If (BU_EXECUTION_GRAPH_SCRATCH_BIT_AMDX  in Value)       then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_EXECUTION_GRAPH_SCRATCH_BIT_AMDX) ;
        If (BU_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT  in Value)   then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT) ;
        If (BU_TILE_MEMORY_BIT_QCOM in Value)                          then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_TILE_MEMORY_BIT_QCOM) ;
        If (BU_DESCRIPTOR_HEAP_BIT_EXT  in Value)                      then Result := Result or TVkBufferUsageFlags(VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_EXT) ;
      ...
      Begin
        ...
        If TestValue(VK_BUFFER_USAGE_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT)        then include(Result, BU_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT) ;


        If TestValue(VK_BUFFER_USAGE_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT)        then include(Result, BU_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT) ;
        If TestValue(VK_BUFFER_USAGE_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT)        then include(Result, BU_MICROMAP_BUILD_INPUT_READ_ONLY_BIT_EXT) ;
        If TestValue(VK_BUFFER_USAGE_MICROMAP_STORAGE_BIT_EXT)        then include(Result,BU_MICROMAP_STORAGE_BIT_EXT ) ;
        If TestValue(VK_BUFFER_USAGE_EXECUTION_GRAPH_SCRATCH_BIT_AMDX)        then include(Result, BU_EXECUTION_GRAPH_SCRATCH_BIT_AMDX) ;
        If TestValue(VK_BUFFER_USAGE_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT)        then include(Result,BU_PUSH_DESCRIPTORS_DESCRIPTOR_BUFFER_BIT_EXT) ;
        If TestValue(VK_BUFFER_USAGE_TILE_MEMORY_BIT_QCOM)        then include(Result, BU_TILE_MEMORY_BIT_QCOM) ;
        If TestValue(VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_EXT)        then include(Result, BU_DESCRIPTOR_HEAP_BIT_EXT) ;
      End;
      ...
      Begin
        ...
        If (IU_RESERVED_16_BIT_HUAWEI in Value)             then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_16_BIT_HUAWEI);
        If (IU_RESERVED_27_BIT_HUAWEI in Value)             then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_27_BIT_HUAWEI);
      ...
      //  If (IU_SHADING_RATE_IMAGE_BIT_NV in Value)        then Result:= Result + TVkImageUsageFlags(VK_IMAGE_USAGE_SHADING_RATE_IMAGE_BIT_NV);
        ...
        If (IU_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT in Value)       then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT);
        If (IU_SAMPLE_WEIGHT_BIT_QCOM in Value)                  then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM);
        If (IU_SAMPLE_BLOCK_MATCH_BIT_QCOM in Value)              then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM);
        If (IU_HOST_TRANSFER_BIT in Value)                        then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_HOST_TRANSFER_BIT);
        If (IU_TENSOR_ALIASING_BIT_ARM in Value)                  then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_TENSOR_ALIASING_BIT_ARM);
        If (IU_RESERVED_24_BIT_COREAVI in Value)                   then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_24_BIT_COREAVI);
        If (IU_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR in Value)       then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR);
        If (IU_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR in Value)       then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR);
        If (IU_TILE_MEMORY_BIT_QCOM in Value)                    then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_TILE_MEMORY_BIT_QCOM);
        If (IU_RESERVED_28_BIT_EXT in Value)                      then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_28_BIT_EXT);
        If (IU_RESERVED_29_BIT_KHR in Value)                       then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_29_BIT_KHR);
        If (IU_RESERVED_30_BIT_KHR in Value)                        then Result:= Result OR TVkImageUsageFlags(VK_IMAGE_USAGE_RESERVED_30_BIT_KHR);
      ...
      Begin
        ...
        If TestValue(VK_IMAGE_USAGE_RESERVED_16_BIT_HUAWEI)             then Include(Result , IU_RESERVED_16_BIT_HUAWEI);
        If TestValue(VK_IMAGE_USAGE_RESERVED_27_BIT_HUAWEI)             then Include(Result , IU_RESERVED_27_BIT_HUAWEI);
        If TestValue(VK_IMAGE_USAGE_INVOCATION_MASK_BIT_HUAWEI)        then Include(Result , IU_INVOCATION_MASK_BIT_HUAWEI);

        If TestValue(VK_IMAGE_USAGE_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT)        then Include(Result , IU_ATTACHMENT_FEEDBACK_LOOP_BIT_EXT);
        If TestValue(VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM)                  then Include(Result , IU_SAMPLE_WEIGHT_BIT_QCOM);
        If TestValue(VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM)             then Include(Result , IU_SAMPLE_BLOCK_MATCH_BIT_QCOM);
        If TestValue(VK_IMAGE_USAGE_HOST_TRANSFER_BIT)                        then Include(Result , IU_HOST_TRANSFER_BIT);
        If TestValue(VK_IMAGE_USAGE_TENSOR_ALIASING_BIT_ARM)                  then Include(Result , IU_TENSOR_ALIASING_BIT_ARM);
        If TestValue(VK_IMAGE_USAGE_RESERVED_24_BIT_COREAVI)                  then Include(Result , IU_RESERVED_24_BIT_COREAVI);
        If TestValue(VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR)        then Include(Result , IU_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR);
        If TestValue(VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR)        then Include(Result , IU_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR);
        If TestValue(VK_IMAGE_USAGE_TILE_MEMORY_BIT_QCOM)                     then Include(Result , IU_TILE_MEMORY_BIT_QCOM);
        If TestValue(VK_IMAGE_USAGE_RESERVED_28_BIT_EXT)                      then Include(Result , IU_RESERVED_28_BIT_EXT);
        If TestValue(VK_IMAGE_USAGE_RESERVED_29_BIT_KHR)                      then Include(Result , IU_RESERVED_29_BIT_KHR);
        If TestValue(VK_IMAGE_USAGE_RESERVED_30_BIT_KHR)                      then Include(Result , IU_RESERVED_30_BIT_KHR);
...
*)
      ...
      Begin
        // Use RTTI to fetch the enum identifier name as declared in the Vulkan unit.
          ...
          Result := GetEnumName(TypeInfo(TvgFormat), Ord(Value));
...
function TRenderPassKey.ComputeHash: Cardinal;
var
  I: Integer;
  Hash: Cardinal;
begin
  Hash := AttachmentCount xor SubpassCount;
  for I := 0 to High(AttachmentFormats) do
    Hash := Hash xor Cardinal(AttachmentFormats[I]);
  Result := Hash;  // Use a better hash algo like MurmurHash for productionend;
end;
